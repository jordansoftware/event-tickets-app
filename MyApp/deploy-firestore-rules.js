...
if (!fs.existsSync('firebase.json')) {
  ...
  execSync('firebase init firestore --project myapp-5d335 --yes', { stdio: 'inherit' });
}

// Déployer les règles
console.log('📤 Déploiement des règles Firestore...');
try {
  execSync('firebase deploy --only firestore:rules --project myapp-5d335', { stdio: 'inherit' });
  console.log('✅ Règles Firestore déployées avec succès !');
} catch (error) {
  console.error('❌ Erreur lors du déploiement:', error.message);
  console.log('💡 Vous pouvez aussi déployer manuellement avec:');
  console.log('   firebase deploy --only firestore:rules --project myapp-5d335');
} 